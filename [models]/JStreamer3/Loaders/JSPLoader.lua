	...
	if iA == 1 then
		...
		XA,YA,ZA = tonumber(x),tonumber(y),tonumber(z)
			...
			if not (SplitA[1] == '!') then -- IF #1 == ! THEN IGNORE
				local object = exports.JStreamer3:JcreateObject(SplitA[1],tonumber(SplitA[4])+XA,tonumber(SplitA[5])+YA,tonumber(SplitA[6])+ZA,tonumber(SplitA[7]),tonumber(SplitA[8]),tonumber(SplitA[9]))
			if object then
